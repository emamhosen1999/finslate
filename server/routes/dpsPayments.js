...
router.get('/', requireAuth, async (req, res, next) => {
  try {
    ...
    let query = `
      SELECT dp.*, d.institution_name as dps_name, d.installment_amount, a.name as account_name
...
router.post('/', requireAuth, async (req, res, next) => {
  ...
  try {
    ...
    if (account_id) {
      const [accounts] = await connection.query('SELECT id FROM accounts WHERE id = ? AND user_id = ?', [account_id, req.user.id]);
    ...
    const [result] = await connection.query(
      `INSERT INTO dps_payments 
       (dps_id, installment_number, due_date, paid_date, amount, account_id, status)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [dps_id, installment_number, due_date, paid_date, amount, account_id, status || 'upcoming']
    );
    ...
    if (account_id && status === 'paid') {
      await connection.query(
        `INSERT INTO transactions (user_id, account_id, type, amount, currency, transaction_date, category_id, source_type, source_id, notes)
         VALUES (?, ?, 'expense', ?, 'BDT', ?, 'Savings', 'account', ?, ?)`,
        [req.user.id, account_id, amount, paid_date || new Date().toISOString().slice(0,10), result.insertId, 'DPS installment']
      );
...
router.put('/:id', requireAuth, async (req, res, next) => {
  ...
  try {
    ...
    await connection.query(
      `UPDATE dps_payments 
       SET installment_number = ?, due_date = ?, paid_date = ?, amount = ?,
           account_id = ?, status = ?
       WHERE id = ?`,
      [installment_number, due_date, paid_date, amount, account_id, status, req.params.id]
    );
